//! Hash Time-Locked Contracts (v0.0.18).
//!
//! HTLCs are the building block for trustless atomic swaps between AII
//! and external chains. A sender locks funds against a secret hash;
//! the recipient claims by revealing the preimage before the timelock
//! expires, otherwise the sender refunds after expiry.
//!
//! This module provides the **on-chain state machine** only — no asset
//! movement, no source-chain listener, no broker daemon.
//!
//! ## State machine
//!
//! ```text
...
//! ## Hash function
//!
//! AII uses Keccak-256 throughout. Cross-chain peers that prefer
//! SHA-256 (e.g. Bitcoin / classic HTLCs) MUST agree on the digest in
//! their swap protocol; this module only validates Keccak-256.
