...
impl LocalWallet {
    ...
    /// 32-byte secret-key view. Use sparingly — exposing the secret is
    /// what the encrypted keystore is meant to prevent.
...
impl Drop for LocalWallet {
    fn drop(&mut self) {
        // SecretKey wraps k256's SigningKey which already zeroes on drop —
        // we also wipe the address copy as cheap defence in depth.
