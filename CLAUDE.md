...
| **代币模型** | 预约启动：60 天公开登记 → 10,000 名额 × 100 AII = 4.76% 创世；SECRC 多签 5 万 AII = 0.24%；剩余 95% 通过 PoS 出块释放 |
| **总量上限** | 21,000,000 AII（沿用比特币 2100 万哲学，**协议常量、不可变更**） |
| **开发语言** | Rust 全栈（含 revm / blst / arkworks / RocksDB / wasmtime） |
| **V 节点抵押 S_min** | **50 AII**（不是 50,000——已下调以适配预约启动） |
| **区块奖励分配** | 80% 提议者 / 20% PRE-COMMIT 见证者（**无 DAO 国库份额**、**无保险基金份额**） |
| **协议升级** | Bitcoin / Linux 式 rough consensus —— 节点运营者自愿升级；**没有链上投票通道** |
| **SECRC** | 7 名 genesis 多签持有者，仅负责紧急安全热补丁 + 5 万 AII 小额运营（**无任何治理决策权**） |
